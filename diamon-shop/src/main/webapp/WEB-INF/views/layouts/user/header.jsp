...
<header id="header">
	<div class="row">
		...
		<div class="span4 alignR">
			...
			<a class="btn" href="<c:url value='/gio-hang/chi-tiet-gio-hang'/>">
			
			<span class="btn btn-mini">
			<c:if test="${not empty sessionScope.size}">
			[ ${sessionScope.size} ] 
			</c:if>
			<c:if test="${empty sessionScope.size}">
			[ 0 ] 
			</c:if>		
			...
			<div class="nav-collapse">
				...
				<c:forEach var="item" items="${menus}" varStatus="index">
					...
					<c:if test="${not index.first}">
						...
						<c:if test="${item.name eq 'Giỏ hàng'}">
						<a href="<c:url value='/gio-hang/chi-tiet-gio-hang'/>">${item.name}</a></li>
						</c:if>
						<c:if test="${item.name ne 'Giỏ hàng'}">
